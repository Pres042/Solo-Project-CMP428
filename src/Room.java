...
public abstract class Room {
	  ...
	  Dimension screenSize = toolkit.getScreenSize(); screenWidth = screenSize.width; screenHeight = screenSize.height-75; }
	  
	  
	  double scaleX = (double) screenWidth / ORIGINAL_SCREEN_WIDTH; 
	  double scaleY = (double) screenHeight / ORIGINAL_SCREEN_HEIGHT;
	 
	BufferedImage backBuffer = new BufferedImage(screenWidth, screenHeight, BufferedImage.TYPE_INT_ARGB);
	Graphics2D backBufferGraphics = backBuffer.createGraphics();

	static Room[] room = new Room[100];
	...
	static Lord lord = new Lord(200, 200, 100, 100);
	//gotta change the jump logic
	static boolean lCanJump = true;
	static boolean[] pressing;

	ImageLayer background;

	public Room(boolean[] pressing, String filename) {
	...
	public static void paintStatic(Graphics pen) {
		room[0].paint(pen);
	}
